/*
  circular_linked_list_delete_head.cpp
  ...
  Key Terminologies:
    ...
    • deleteAtHead: Operation to remove the node at the beginning (head) of the list.
    • Single Node Case: When head == tail, deleting head empties the list.
    • Pointer Maintenance: Ensuring tail->next always points to head after changes.
 
  Usage:
    1. Compile: g++ circular_linked_list_delete_head.cpp -o circular_delete_head
    2. Run:     ./circular_delete_head
 */
...
public:
    ...
    // deleteAtHead: Remove the node at the front of the list
    void deleteAtHead() {
        if (!head) {
            ...
            // Multiple nodes: remove head, update head and tail->next
            Node* temp = head;
